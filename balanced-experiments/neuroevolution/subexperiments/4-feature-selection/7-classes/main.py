...
if __name__=='__main__':
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        scikitplot.metrics.plot_confusion_matrix(y[test_index], outputs_test, title="Seven Classes Confusion Matrix: Fold " + str(n_fold) , normalize=True, text_fontsize="small")
        plt.savefig("7-classes-cf-" + str(n_fold) + ".png")
        plt.clf()
        plt.close()

        vs.draw_net(config, winner, False, filename='7-classes-winner-' + str(n_fold), show_disabled=False)
        with open('7-classes-winner-' + str(n_fold), 'wb') as f:
            pickle.dump(winner, f)
        
        with open('7-classes-results-' + str(n_fold), 'w') as f:
            f.write('\nBest genome:\n{!s}'.format(winner))
    ...
    with open("results-7-classes.csv", 'w', encoding='UTF8', newline='') as csvfile:
