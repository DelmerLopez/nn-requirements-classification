...
if __name__=='__main__':
    x = pickle.load(open('../../../../datasets/x-no-0', 'rb'))
    y = pickle.load(open('../../../../datasets/y-no-0', 'rb'))
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        scikitplot.metrics.plot_confusion_matrix(y[test_index], outputs_test, title="Three Classes Confusion Matrix: Fold " + str(n_fold) , normalize=True, text_fontsize="small")
        plt.savefig("3-classes-cf-" + str(n_fold) + ".png")
        plt.clf()
        plt.close()

        vs.draw_net(config, winner, False, filename='3-classes-winner-' + str(n_fold), show_disabled=False)
    ...
    with open("results-3-classes.csv", 'w', encoding='UTF8', newline='') as csvfile:
