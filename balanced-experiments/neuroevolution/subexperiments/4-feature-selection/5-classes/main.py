...
if __name__=='__main__':
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        plt.savefig("5-classes-cf-" + str(n_fold) + ".png")
        plt.clf()
        plt.close()

        vs.draw_net(config, winner, False, filename='5-classes-winner-' + str(n_fold), show_disabled=False)
        with open('5-classes-winner-' + str(n_fold), 'wb') as f:
            pickle.dump(winner, f)
        
        with open('5-classes-results-' + str(n_fold), 'w') as f:
            f.write('\nBest genome:\n{!s}'.format(winner))
